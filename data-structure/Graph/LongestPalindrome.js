...
function manacher(s) {
    // 填充字符变成奇数字符窜
    var expandedStr = '#';
    var resolveStr = expandedStr;
    for (var i = 0; i < s.length; i++) {
        resolveStr += s[i];
        resolveStr += '#';
    }
    // p[i] i位置回文子串的回文半径
    var p = new Array(resolveStr.length).fill(1);
    // 当前最右边回文子串的中心索引
    var rightSide = 0;
    // 当前最右边回文子串的右边界索引
    ...
    var maxIndex = -1;
    var maxLen = 0;
    for (var i = 0; i < resolveStr.length; i++) {
        // 如果当前位置为中心的回文在右边界的左边
        if (i < rightSide) {
    ...
    var str = resolveStr.substring(maxIndex - maxLen + 1, maxIndex + maxLen);
