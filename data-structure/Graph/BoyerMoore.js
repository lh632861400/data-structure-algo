...
function bm(text, pattern) {
    ...
    var badCharacterHeuristic = preProcessToBuildBadCharacterHeuristic(pattern);
    // 构建好后缀启发式跳转表
    // i模式字符窜失配字符的索引
    // goodSuffixHeuristic[i]当前失配索引i需要移动的长度
    var goodSuffixHeuristic = preProcessToBuildGoodSuffixHeuristic(pattern);
    // 进行匹配
    ...
    function preProcessToBuildBadCharacterHeuristic(pattern) {
        // 字符
        var n = 256;
        var m = pattern.length;
        // 下标代表字符
        ...
        // 这个构建的不是真正的坏字符表
        var badCharacterHeuristic = new Array(n).fill(m);
        for (var i = 0; i < m - 1; i++) {
            badCharacterHeuristic[pattern.codePointAt(i)] = m - 1 - i;
        }
        return badCharacterHeuristic;
    }
    function preProcessToBuildGoodSuffixHeuristic(pattern) {
        var m = pattern.length;
        var suffix = suffixArr(pattern);
        // 下标代表位置
        // goodSuffixHeuristic[j]代表失配位置j需要移动的长度
        var goodSuffixHeuristic = new Array(m).fill(m);
        // case3
        for (var i = 0; i < m; i++) {
    ...
    function suffixArr(pattern) {
        var m = pattern.length;
        // 以i为终点字符的最长的公共后缀长度
        var suffix = new Array(m).fill(m);
        suffix[m - 1] = m;
        for (var i = m - 2; i >= 0; i--) {
            var j_1 = i;
            // i - j 移动的距离
            // m - 1 - (i - j)当前正在匹配的索引
            while (j_1 >= 0 && pattern[j_1] === pattern[m - 1 - (i - j_1)]) {
                j_1--;
            }
            suffix[i] = i - j_1;
        }
